...
def get_ip_webcam_image(ip_webcam_url):
    try:
        ...
        print(f"Error fetching image from IP webcam: {str(e)}")
    return None
...
for cl in mylist:
    if cl.endswith(('.jpg', '.jpeg', '.png', '.gif')):
        ...
        if curImg is not None:
            ...
            print(f"Name: {name}, Roll Number: {roll_number}, Section: {section}") 
            classNames.append((name, roll_number, section))

encodelistknown = find_encodings(images)
print('Encoding Complete!')

ip_webcam_url = 'http://192.168.201.68:8080/photo.jpg'

attendance_status = {}
previous_names = set()

while True:
    img = get_ip_webcam_image(ip_webcam_url)
